...
than some number of students are present when class starts. Arrival times go from on time (arrivalTime<=0) to arrived late (arrivalTime>0).
...
Note: Non-positive arrival times (a[i]<=0) indicate the student arrived early or on time; positive arrival times (a[i]>0) indicate the student arrived a[i] minutes 
late.
...
The second line contains n space-separated integers (a[n],a[n],.....,a[n]) that describe the arrival times for each student.

Constraints-
1<=t<=100
1<=n<=1000
1<=k<=n
-100<=a[i]<=100,where i belongs [1,....n]
