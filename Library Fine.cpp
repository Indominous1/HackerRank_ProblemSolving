...
3. If the book is returned after the expected return month but still within the same calendar year as the expected return date,fine=500 Hackos *(number of months late)
4. If the book is returned after the calendar year in which it was expected, there is a fixed fine of 10000 Hackos.
                                                                                           
Charges are based only on the least precise measure of lateness. For example, whether a book is due January 1, 2017 or December 31, 2017, if it is returned 
...
> 1<=m1,m2<=12
> 1<=y1,y1<=3000
> Its is guaranteed that these dates will valid in Gregogrian calender dates.
...
Per the library's fee structure, we know that our fine will be 15 Hackos*(# days of late). We then print the result of 15*(d1-d2)=15*(9-6)=45 as our output.
...
int main()
{
    ...
    else if(y2==y1 && d1>d2 && m1==m2)          //day
    {
      cout<<(15*(d1-d2));    
    }
    else if(y2==y1 && m1>m2)                    //month
    {
       cout<<500*(m1-m2);
    }
    else {
